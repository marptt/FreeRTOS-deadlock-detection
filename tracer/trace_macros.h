...
#define xTaskCreate( pxTaskCode, pcName, usStackDepth, pvParameters, uxPriority, pxCreatedTask) xTaskCreate_scp(pxTaskCode, pcName, usStackDepth, pvParameters, uxPriority, pxCreatedTask, GET_SOURCE_CODE_POSITION )
...
#define traceTASK_CREATE_DEADLOCK(xTask) onTraceTaskCreate(xTask, source_code_position)
...
#define traceTASK_SWITCHED_IN() onTraceTaskSwitchedIn(pxCurrentTCB)
// #define traceTASK_SWITCHED_OUT() onTraceTaskSwitchedOut(pxCurrentTCB)
